...
# Authenticate using GitHub Secrets (set in repo settings)
auth = OAuth1(
...
# Static BrickLink Color ID to Color Name mapping
color_lookup = {
...
def get_inventory():
    all_items = []
    for page in range(1, 2):  # Start with 1 page for testing
...
with open("meta_product_feed.csv", "w", newline='', encoding="utf-8") as f:
    ...
    for item in inventory:
        ...
        color_name = color_lookup.get(color_id, f"Color ID {color_id}")

        title = f"{color_name} {name}"
        condition = "New" if item["new_or_used"] == "N" else "Used (like new)"

        # BrickLink standard image link format
        image_link = f"https://img.bricklink.com/ItemImage/PL/{color_id}/{part_no}.png"

        # Direct search URL to that inventory_id
        product_link = f"https://store.bricklink.com/luke.donohoe#/shop?o={{\"q\":\"{item['inventory_id']}\",\"sort\":0,\"pgSize\":100,\"showHomeItems\":0}}"
...
# Auto-refresh for public download
